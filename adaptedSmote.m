...
smoteData = zeros(size(minorityData, 1), size(minorityData, 2));
...
for i = 1:size(IDX, 1)
     ...
     r = randi([1 5],1,1);
        ...
        if class(IDX(i, r), 1) == 1 %It belongs to the minor class 
            smoteData(i, :) = minorityData(i, :) + (rand * (data(IDX(i, r), :) - minorityData(i, :)));
        else  %It belongs to the major class
            smoteData(i, :) = minorityData(i, :) + (rand/2 * (data(IDX(i, r), :) - minorityData(i, :)));
        end
end
end
